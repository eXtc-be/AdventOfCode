# aoc_2017_24_A_3.py - Day 24: Electromagnetic Moat - part 1
# What is the strength of the strongest bridge you can make with the components you have available?
# https://adventofcode.com/2017/day/24
# this version runs faster still by doing all the validating in the combinations generator
# unfortunately that is still not fast enough when running with the real input..
    ...
    for i in range(len(components)):
        ...
        for combo in permutations(components, i+1):
            if 0 in combo[0].ports:
                # check if every pair of components connects to each other
                used = 0
                for comp_a, comp_b in zip(combo, combo[1:]):
                    # find the connecting port
                    if comp_a.ports[0] == comp_a.ports[1]:  # 2 identical ports, pick any one as the one being used next
                        used = comp_a.ports[0]
                    else:  # 2 different ports, pick the one that is not previously used as the one being used next
...
def main(data_lines: list[str]) -> None:
    ...
    valid_bridges = [Bridge(list(combo)) for combo in _combinations(components)]
