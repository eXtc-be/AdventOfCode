...
# By comparing the outputs of my program with the output of extern_2 I found that in step 24 the Goblin at (15,9),
# trying to reach the empty spot at (22,10) went the wrong way. After fixing that, my program found a correct
...
CAPTURE = True
BOLD = '' if CAPTURE else '\033[1m'
ENDC = '' if CAPTURE else '\033[0m'
...
class Point(NamedTuple('Point', [('r', int), ('c', int)])):
    ...
    @property
    def neighbours(self):
        return [self + d for d in [Point(-1, 0),Point(0, -1),  Point(0, 1), Point(1, 0), ]]
...
class Grid(dict):
    ...
    def move(self, unit_position: Point, no_elf_losses: bool = False, confirm_next_step: bool = False) -> bool:
        ...
        adjacent_positions = set(
            ...
            if self[point].type in TEAMS and not self[point].alive or
                self[point].type == Type.EMPTY
                or point == unit_position
        )
    ...
    def find_move(self, unit_position, adjacent_positions):
        ...
        while visiting:
            ...
            for neighbour in pos.neighbours:
                ...
                if neighbour not in meta or meta[neighbour][0] > dist + 1:
...
def clear() -> None:
    os.system('cls' if os.name == 'nt' else 'clear') if not os.environ.get('CHARM') and not CAPTURE else print('-' * 100)
...
if __name__ == "__main__":
    ...
    main(load_data('input_2018_15_6'), False)  # 78 * 2843 = 221754
    ...
    # using input data:
    #   End result: Goblins win after 81 full rounds with 2751 hit points left. Score: 81 * 2751 = 222831
    #   Finished 'main' in 3.4 seconds
    # using input data 2:
    #   End result: Goblins win after 82 full rounds with 2459 hit points left. Score: 82 * 2459 = 201638
    #   Finished 'main' in 2.3 seconds
    # using input data 3:
    #   End result: Goblins win after 103 full rounds with 2416 hit points left. Score: 103 * 2416 = 248848
    #   Finished 'main' in 2.2 seconds
    # using input data 4:
    #   End result: Goblins win after 67 full rounds with 2922 hit points left. Score: 67 * 2922 = 195774
    #   Finished 'main' in 2.1 seconds
    # using input data 5:
    #   End result: Goblins win after 76 full rounds with 2720 hit points left. Score: 76 * 2720 = 206720
    #   Finished 'main' in 2.7 seconds
    # using input data 6:
    #   End result: Goblins win after 78 full rounds with 2843 hit points left. Score: 78 * 2843 = 221754
    #   Finished 'main' in 2.9 seconds
